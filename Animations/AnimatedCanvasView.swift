...
struct AnimatedCanvasView: View {
    ...
    var body: some View {
        ...
        KeyframeAnimator(
            ...
            content: { color in
                /*
                 Docs say "The content closure updates every frame while animating, so avoid
                 performing any expensive operations directly within content.", but performance
                 seems good enough for animating input values of Canvas drawing.
                 */
                Canvas { context, _ in
                    context.fill(
